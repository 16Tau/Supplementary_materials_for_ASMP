...
from torchvision.models import alexnet,vgg16,googlenet,densenet121,resnet18
import torch
...
# Add the parent directory to sys.path to import AlexNet
...
def untarget_index_image_path(image_path, index_path, weight_path, index, model_cnn):
    ...
    # Create model
    model = model_cnn(num_classes=4).to(device)
    # Load model weights
    ...
    top_probs, top_indices = torch.topk(classification_probability, 4)
    attack_index = top_indices[1]
    return(attack_index)
...
# Strategy 1: Only use forward iterative tampering
def main_strategy1(t):
    ...
    image_files = [f for f in os.listdir(actual_images_folder_absolute_path) if f.lower().endswith(('png', 'jpg', 'jpeg'))]
    ...
    for image_file in image_files:
        ...
        iterative_image_matrix_forward = np.zeros((max_num_iterative + 2, 3, 224, 224), dtype=np.float64)
        ...
        iterative_image_pixel_dot_ischange = np.zeros((max_num_iterative + 2, 3, 224, 224), dtype=np.float64)
        ...
        iterative_image_label = np.zeros(max_num_iterative + 2, dtype=int)
    ...
    ##########################################################################################################
            # Strategy 1: Skip forward calibration step, directly use the tampered image
            iterative_image_path = "No target attack image.png"

            # Input the positively monotonically tampered images of this round into the model for category judgment
...
# Strategy 1+2: Use forward iterative tampering + forward calibration
def main_strategy1_2(t):
...
if __name__ == '__main__':
    print("Running strategy 1 (only forward iterative tampering)...")
    main_strategy1(2)
    print("\nRunning strategy 1+2 (forward iterative tampering + forward calibration)...")
    main_strategy1_2(2)